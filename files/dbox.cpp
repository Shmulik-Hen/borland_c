...
void draw_dbox()
{
 ...
 while(!done)
   {
    while(!kbhit() && !MousePressed());                      //wait for action
    if(kbhit())                                           //keyboard selection
       ...
       switch(c)
	 {
	  ...
	  case 'N':do_no();                                  //do it
		   ...
		   break;
	  case '\r':if(yes) do_yes();                      //ENTER was pressed
		   else do_no();                           //perform selection
		   done=1;
		   break;
	  case 77: if(yes)                                       //right arrow
		     {                                             //select NO
		      yes=0;
		      HideMouse();
	  ...
	  case 75: if(!yes)                                       //left arrow
		   ...
		   break;
	  case 27: HideMouse();                           //ESCAPE was pressed
       ...
       if(between(p,124,160,164,175))                           //YES selected
	 {
	  ...
	  while(MousePressed());             //keep highlighting until release
	  do_yes();                                                    //do it
	  done=1;
	 }
