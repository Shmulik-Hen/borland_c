 ...
 *                                                                         *
 * PURPOSE:    function definitions for fast SVGA graphics function.       *
 **************************************************************************/
...
#define update_screen(buff) movedata(FP_SEG(buff), FP_OFF(buff), VGA_SEG, 0, VGA_SIZE)

/* function for setting the current fill color */
...
/*function for drawing a line from source point to destination */
...
/*function for drawing a line from current point to destination */
...
void lineto(int x, int y)
{
	...
	if (dx > dy)
		while (loopx--) {
			if ((G_POS_X <= vp_max_x) && (G_POS_Y <= vp_max_y) && (G_POS_X >= vp_min_x) && (G_POS_Y >= vp_min_y))
...
/* function for drawing a filled polygon */
...
void fillpoly(int numpoints, int far *polypoints)
{
	...
	while (--numpoints) {
		...
		if (min_x > polypoints[idx])
			min_x = polypoints[idx];
		if (max_x < polypoints[idx])
			max_x = polypoints[idx];
		idx++;
		if (min_y > polypoints[idx])
			min_y = polypoints[idx];
		if (max_y < polypoints[idx])
			max_y = polypoints[idx];
		idx++;
	}
	...
	if (((min_x < vp_min_x) && (max_x > vp_max_x)) || ((min_y < vp_min_y) && (max_y > vp_max_y)))
		return;
	...
	for (i = min_y; i <= max_y; i++) {
		...
		if (pix_on)
			bar(start_x, i, end_x, i);
	}
}

/* function for clearing the graphics device */
...
/* function for putting a line of text at X,Y */
...
/* function for puting a single character at X,Y */
...
/* function for initializing the graphic device to 320x200x256 mode */
...
/* function for restoring the graphic device to text mode */
...
/* function for setting the active view port */
...
/* function for clearing the view port */
...
/* function for initializing the color palette */
...
/* function for reading the current palette */
...
/* function for writing the new palette */
...
/*function for updating the screen from the temporary drawing buffer */
