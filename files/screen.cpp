...
{        
 ...
 int opt;
 char ch;
 char *menu_opt[3][6]={{"OPEN","NEW","SAVE","SAVE AS","PRINT","QUIT"},
		 {"CUT","COPY","PASTE","CLEAR"},
		 {"FIND","REPLACE","GO TO","SEARCH"}};
 char *str[2]={"FILE       EDIT        SEARCH",
 ...
 do{
    ...
    switch(ch)
	     {
	      case 'f':opt=m[0].get_opt(1,2,menu_opt[0]);
		       gotoxy(1,25);
		       printf("%c-%d",ch,opt);	
		       break;
	      case 'e':opt=m[1].get_opt(10,2,menu_opt[1]);
		       gotoxy(1,25);
		       printf("%c-%d",ch,opt);		
		       break;	
	      case 's':opt=m[2].get_opt(23,2,menu_opt[2]);
		       gotoxy(1,25);
		       printf("%c-%d",ch,opt);		
		       break;	
	     }
